...
Now includes option to use new modular implementation.
"""
import os
import json
...
# Option to use new modular implementation
USE_NEW_IMPLEMENTATION = os.environ.get("USE_NEW_AGENT_GRAPH", "false").lower() == "true"

if USE_NEW_IMPLEMENTATION:
    print("[INFO] Using new modular agent graph implementation")
    from agents.agent_graph_new import process_command_stream as new_process_command_stream
    
    def process_command_stream(message: str):
        """Delegate to new implementation if enabled."""
        return new_process_command_stream(message)
else:
    print("[INFO] Using legacy agent graph implementation")
    def process_command_stream(message: str):
        """
